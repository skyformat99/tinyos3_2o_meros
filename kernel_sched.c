...
/*
  ...
  *** MUST BE CALLED WITH sched_spinlock HELD ***
*/
static void sched_queue_add(TCB* tcb,int priority)
{
...
static void sched_make_ready(TCB* tcb)
{
    ...
    if(tcb->phase == CTX_CLEAN) {
        if(tcb->priority>=1)
            tcb->priority--;
        sched_queue_add(tcb,tcb->priority);
    }
}
...
/** Put starving threads of a low priority queue, to a higher priority queue.  **/
void boost(int pos){
    rlnode * temp = NULL;
    while(! is_rlist_empty(&SCHED[pos])) {
        temp = rlist_pop_back(&SCHED[pos]);
        rlist_push_front(&SCHED[pos-1], & temp->tcb->sched_node);
    }
}
...
static TCB* sched_queue_select(){
    ...
    /** We boost the low priority queues when needed, to prevent threads from starving**/
    for(int i=2; i<=QUEUE_LEVELS; i++){
        if((int)rlist_len(&SCHED[i]) >=30) {
    ...
    Now we have a table of queues, so we're looking for the first node of all the queues **/
    rlnode* sel = NULL;
    ...
    return NULL;   //When the list is empty, we must return NULL
}
...
void yield(enum SCHED_CAUSE cause)
{
    ...
    switch (cause){
        /** Quantum reached before the end of the process **/
        /** Lower the priority of such threads **/
        case SCHED_QUANTUM:
            if(current->priority<QUEUE_LEVELS-1) current->priority ++;
            break;
        /** The thread called yield() because its mutex was locked by another thread**/
        /** We inverse the priority of the thread which called yield() by lowering its priority until the thread which uses the mutex exits **/
        case SCHED_MUTEX:
            if(current->priority<QUEUE_LEVELS-1) current->priority ++;
            break;
        /** Threads that call yield() for IO are prioritized**/
        case SCHED_IO:
...
void gain(int preempt)
{
    ...
    if(current != prev) {
        ...
        {
            case READY:
                if(prev->type != IDLE_THREAD) sched_queue_add(prev,current->priority);
                break;
            /** When a thread wants to exit, the memory is freeed, unless it's the main thread. **/
            case EXITED:
                if(prev->ptcb_parent->iam_mainthread != 1) prev->ptcb_parent->state = EXITTED;
    ...
    /** Set a quantum alarm for each queue **/

    if (CURTHREAD->priority==0)
        bios_set_timer(QUANTUM*0.02);
    else
        bios_set_timer(QUANTUM*1.5*CURTHREAD->priority);
