...
Pid_t sys_Exec(Task call, int argl, void* args){
   ...
   **/
  if(call != NULL) {
      newproc->threads_counter ++;

      newproc->main_thread = spawn_thread(newproc, start_main_thread);      /** Create main thread of proccess **/
      newproc->main_thread->ptcb_parent = initialize_main_PTCB();              /** Init main PTCB thread changing "iam_mainthread" PTCB variable to 1,
                                                                                              so we know that refers to main PTCB thread**/
